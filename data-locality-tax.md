...
origins:
  - do-internal
---
Your [RAG pipeline's](https://www.digitalocean.com/community/tutorials/end-to-end-rag-pipeline) latency conversation is probably stuck on the wrong question. Teams benchmark GPUs, tune HNSW(HNSW-Algo) parameters, argue about `ef_search` values, and shave single-digit milliseconds off approximate [nearest neighbor search](https://www.digitalocean.com/community/tutorials/k-nearest-neighbors-knn-in-python). Meanwhile the vector database holding their embeddings sits in a different region from the GPU running their model, and every single retrieval call pays a round-trip tax set by the distance between two buildings. No index parameter recovers that time. The speed of light does not have a config flag.
...
> **Where the numbers in this piece come from.** Two sources, kept strictly apart. The round-trip floors (5.5 ms, 41.3 ms, 62.0 ms, 153.3 ms and the rest) are derived from great circle distance divided by the propagation speed of light in optical fiber, roughly 200,000 km per second. They are lower bounds set by physics, not measurements, and real paths are slower because fiber does not follow great circles. The measured columns come from the published harness that I ran on August 10, 2026 against live [DigitalOcean GPU Droplets](https://www.digitalocean.com/products/gpu-droplets) and [Vector Databases](https://docs.digitalocean.com/products/vector-databases/) (PostgreSQL with pgvector); raw per-cell JSON is in [anishsingh20/data-locality-tax](https://github.com/anishsingh20/data-locality-tax).
...
- **The floor of the tax is checkable arithmetic, and this article shows the work.** A round trip between New York and San Francisco is at least 41.3 ms in fiber, New York to Frankfurt at least 62.0 ms, New York to Singapore at least 153.3 ms, from great-circle distance at roughly 200,000 km per second. These are lower bounds. Real routes are slower. Inside one datacenter, the same floor is below one millisecond.
- **Retrieval blocks time to first token directly.** The model cannot begin prefill until the retrieved context arrives, so the user feels every millisecond of the retrieval path before the first token, on top of everything the [p50 vs p99 Latency](https://www.digitalocean.com/community/tutorials/p50-vs-p99-latency-llm-inference) article already measure about serving.
...
## Table of terms used in this article

If you are new to RAG or networking, skim this table.

| Term | What it means in plain English | Example |
| --- | --- | --- |
| RAG (retrieval-augmented generation) | The app looks up relevant facts first, then asks the language model to answer using those facts, so the model is not guessing from memory alone. | A support bot searches your docs for “refund policy,” then writes the reply from the snippets it found. |
| Retrieval | The lookup step: send a search to the vector database and get matching items back. | One call that returns the five closest document chunks for the user’s question. |
| Vector / embedding | A long list of numbers that represents the *meaning* of text (or another item) so similar ideas land near each other in math space. | The sentence “how do I reset my password?” becomes 768 numbers; a similar help article’s numbers sit nearby. |
| Vector database | A database built to store those number-lists and find the closest matches quickly. | DigitalOcean Managed PostgreSQL with pgvector, OpenSearch, or Weaviate holding your knowledge-base chunks. |
| Data locality tax | Extra wait time you pay on every lookup because the database and the GPU sit far apart: distance you cannot tune away. | GPU in New York, vector DB in San Francisco: about 67 ms per pooled search in the measured run, versus under 2 ms when both are in NYC3. |
| Latency | How long something takes from start to finish, usually in milliseconds (ms). One thousand ms is one second. | A 2 ms same-datacenter search versus a 67 ms cross-region search. |
| Round trip (RTT) | Time for a request to go to the other machine and for the reply to come back. | A TCP probe that only opens a connection and closes it (no database work) still takes ~69 ms NYC→SFO. |
| Latency floor / physics floor | The fastest a path can possibly be, set by the speed of light in fiber and the distance between cities. Real networks are slower. | New York to San Francisco cannot beat about 41.3 ms round trip in fiber, no matter how good your software is. |
| Region / datacenter | A cloud location: a building (or campus) in a city where your machines run. “Same region” means same location family; “same DC” means the same building network. | NYC3 (New York) versus SFO3 (San Francisco) in this experiment. |
| Co-locate / co-location | Put the pieces that talk to each other in the same place on purpose. | Droplet and vector DB both in NYC3 on the same VPC. |
| VPC (virtual private cloud) | A private network inside one DigitalOcean datacenter so your resources talk without going over the public internet. | Arm A queries the database on its private hostname inside NYC3. |
| VPC peering | A paid link that joins two VPCs in different datacenters so traffic can stay private across regions. It does not shrink the distance. | NYC3 peered to SFO3: still ~68 ms RTT: private path, same geography bill. |
| Droplet | A DigitalOcean virtual machine (a cloud computer you rent). A GPU Droplet is one with a GPU for running models. | The harness client `locality-tax-bench-nyc3` in NYC3. |
| LLM (large language model) | The AI model that generates the answer tokens the user reads. | The model waiting on retrieved context before it can start writing. |
| Prefill | The model’s first heavy read of the full prompt (including retrieved text) before it starts producing answer tokens. | Retrieval must finish before prefill can start, so lookup delay shows up before the first word appears. |
| TTFT (time to first token) | How long until the user sees the first piece of the answer. | If retrieval takes 67 ms, that 67 ms is added to TTFT before any token streams. |
| ANN (approximate nearest neighbor) | A fast “close enough” search for similar vectors, instead of checking every item exactly. | The database walking an HNSW index to find near matches. |
| HNSW | A common ANN index shape (layers of neighbor links) used to speed up vector search. | `CREATE INDEX ... USING hnsw` on the `items` table in the runbook. |
| `ef_search` | An HNSW knob: search more carefully (usually slower, sometimes better matches) or more loosely (faster). | Teams tune `ef_search` while the bigger delay is still the cross-region network. |
| top-k | How many nearest matches to return. Larger k means a bigger reply payload. | k=5 versus k=100 in the measured table: more results, more bytes on the return path. |
| pgvector | A PostgreSQL extension that stores vectors and runs similarity search inside Postgres. | The engine used for Arms A, B1, and B2 in the August 10, 2026 run. |
| Corpus | The full set of vectors (and usually the documents behind them) loaded into the database. | 100,000 synthetic 768-number vectors loaded identically into NYC3 and SFO3. |
| Agentic / multi-hop RAG | An agent that retrieves, thinks, retrieves again: several lookups in sequence for one task. | Eight hops × 62 ms ≈ 496 ms of geography before generation, even if each hop looks small alone. |
| TCP | The basic “phone line” setup between two computers on the internet. | The harness TCP probe measures connect time to port 25060 with no SQL query. |
| TLS | Encryption wrapped around that connection (the lock icon in a browser). Setting it up costs extra round trips. | Cold first calls on NYC→SFO landed around 670 to 720 ms because handshakes stack on the long path. |
| Cold connection | A brand-new connection: pay TCP + TLS setup before the first query runs. | Opening a fresh DB connection per serverless request. |
| Connection pooling | Keep and reuse open connections so later queries skip the handshake. | Steady-state pooled k=5 on B1 at ~67 ms versus cold first call at ~718 ms. |
| p50 / p95 / p99 | Percentiles: half of requests are at or below p50; 95% at or below p95; 99% at or below p99. The tail (p99) is what occasional users feel. | Arm A retrieval p50 at k=5 was 1.90 ms; p95 was 3.51 ms. |
| Payload | The data riding on the wire: your query out, results back. Bigger payloads cost more when the path is long. | Returning 100 full documents cross-region versus five IDs. |

## Anatomy of a retrieval round trip

Before measuring anything, let's decompose what one vector search call consists of, because the decomposition is the argument. This is the core distinction I am trying to make in this article: **index tuning optimizes the component placement cannot touch, and placement determines the components tuning cannot touch.** A team benchmarking `ef_search` values while their database sits across a continent is polishing the fast part of a slow path.

A single retrieval call, from the moment your application decides to search until results are usable, spends time in five places.

- **Connection setup**, if no pooled connection exists: TCP and TLS handshakes, paid per new connection.
- **Query serialization**: encoding the query vector and parameters, microseconds, ignorable.
- **Network transit, outbound**: your query crossing the wire to the database.
- **Server-side ANN search**: the index walk itself, the part every tuning guide optimizes.
- **Network transit, return**: the result payload crossing back, and its cost is payload size times available bandwidth plus the same propagation delay, which is why top-k 100 with full documents behaves differently across regions than top-k 5 with IDs.

Sort those five by what controls them. ANN search time responds to index parameters, hardware, and corpus size. Everything else responds to placement and connection discipline.

![Decomposition of one retrieval call into five segments on a timeline: connection setup, query serialization, outbound transit, server-side ANN search, and return transit, with the placement-dependent segments marked in brick red and the tuning-dependent segment marked in teal.](https://doimages.nyc3.cdn.digitaloceanspaces.com/006Community/2026-Community-Tutorials-Anish/DO-CORE-NEW-CONTENT/AI%20Inference%20Tutorials/data-locality-tax/diagram-dl-1-anatomy.png)
...
The reason this lands on user experience directly, rather than hiding in a dashboard is retrieval blocks prefill.

In a standard RAG flow, the model cannot begin processing the prompt until the retrieved context is in hand, so every millisecond of the retrieval path adds to time to first token, the metric the user stares at before anything appears.

## The floor of the tax, derived rather than asserted

Light in optical fiber propagates at roughly [200,000 kilometers per second](https://physics.stackexchange.com/questions/80043/how-fast-does-light-travel-through-a-fibre-optic-cable), about two-thirds of its speed in vacuum, because of the fiber's refractive index. That single number turns distance into a latency floor no engineering removes. Divide the great-circle distance between two cities by 200,000 km/s, double it for the round trip, and you have the minimum possible round-trip time between them, before queueing, before routing detours, before the server does any work at all.

> **Where the numbers in this article come from.** The following table is my own derivation using that method. I computed each great-circle distance with the [haversine formula](https://en.wikipedia.org/wiki/Haversine_formula) and the coordinates of each metro area. These are floors, not estimates of real performance: production fiber routes do not follow great circles, so real round trips exceed every number here.
...
Read the first row against the rest, because the first row is the whole architectural argument. A GPU Droplet and a managed vector database in the same DigitalOcean datacenter, communicating over a [VPC network](https://docs.digitalocean.com/products/networking/vpc/), have a propagation floor three orders of magnitude below any cross-region pairing. DigitalOcean's own [VPC documentation](https://docs.digitalocean.com/products/networking/vpc/) confirms the relevant properties: VPC networks are scoped to a single datacenter region, traffic within a VPC is free and private, and connecting VPCs across datacenters requires [VPC peering](https://docs.digitalocean.com/products/networking/vpc/details/availability/), which is available between all datacenters except **BLR1** and billed at $0.01 per GiB for inter-datacenter traffic. The physics and the pricing point the same direction.

![Bar chart of round-trip latency floors derived from the speed of light in fiber, from under 0.01 ms inside one datacenter through 5.5 ms New York to Toronto, 41.3 ms to San Francisco, 62.0 ms to Frankfurt, and 153.3 ms to Singapore, with a callout marking every bar as a lower bound real routes exceed.](https://doimages.nyc3.cdn.digitaloceanspaces.com/006Community/2026-Community-Tutorials-Anish/DO-CORE-NEW-CONTENT/AI%20Inference%20Tutorials/data-locality-tax/diagram-dl-2-floors.png)
...
This is the section the harness exists for. The design below is fully specified; the harness and raw results have been published in the Github repo I created [The Data Locality Tax — Measured Retrieval Latency Across Regions](https://github.com/anishsingh20/data-locality-tax).

I ran this rest on August 10, 2026. I filled Arms A, B1, and B2 from a NYC3 Droplet against Managed PostgreSQL 16 + pgvector in NYC3 and SFO3 (100,000 synthetic 768-d vectors, HNSW cosine, 75 trials after 10 warmups per cell).

### Fixed variables

One [GPU Droplet](https://www.digitalocean.com/products/gpu-droplets) in a fixed region, NYC recommended, running the embedding model and the LLM. One corpus, roughly 1 million vectors at a realistic dimensionality such as 768 or 1,024, loaded identically into every store under test. Identical index type and parameters across arms wherever the engine allows, so server-side ANN time cancels out of the comparison and the delta isolates the path.

### The three arms

**Arm A, same datacenter over VPC.** A [DigitalOcean Vector Database](https://docs.digitalocean.com/products/vector-databases/) in the same datacenter as the GPU Droplet, attached to the same VPC, queried over the private network. PostgreSQL with pgvector is the safest engine choice because Managed PostgreSQL is broadly available across regions. OpenSearch is the alternative for hybrid search workloads, and [Managed Weaviate](https://docs.digitalocean.com/products/vector-databases/weaviate/) entered public preview on July 1, 2026, per the Vector Databases release notes, so check the [availability page](https://docs.digitalocean.com/products/vector-databases/details/availability/) for your region pair before provisioning it as an arm.

You can refer to the [Choosing Between OpenSearch, Weaviate, and pgvector](https://docs.digitalocean.com/products/vector-databases/concepts/choosing-an-engine/) for more information on the selection tradeoffs.
...
Client: A Droplet(`s-2vcpu-4gb`, Ubuntu 24.04) in **NYC3**. Stores: Managed PostgreSQL 16 + pgvector, plan `db-s-1vcpu-1gb`, identical 100k × 768-d HNSW corpora. Window: 10:04–10:05 UTC. Full JSON: [anishsingh20/data-locality-tax](https://github.com/anishsingh20/data-locality-tax).
...
- **Same building is almost free.** When the Droplet and the database shared NYC3 over the private VPC (Arm A), a typical search for 5 nearest neighbors came back in **1.90 ms**. Half of those calls were that fast or faster. Even the slower ones (p95) were only **3.51 ms**. That is the floor this piece is arguing for.
- **A continent away costs about 35 times more.** The identical search to SFO3 over the public internet (Arm B1) took **66.97 ms** at the median. That is the same query, just traveling to San Francisco and back. A user staring at a blank screen before the first token pays that extra ~65 ms on every retrieval.
- **The database work is not the bottleneck on the long path.** A bare TCP connect to SFO3, with no SQL and no index walk, already took **68.63 ms**. The full retrieval (66.97 ms) sat right next to that number. Once the buildings are far apart, most of the wait is the trip, not HNSW.
- **Physics predicted the ordering, and the run landed above the floor.** Light in fiber cannot beat about **41.3 ms** New York to San Francisco. The measured TCP path sat ~27 ms above that floor, which is expected: real fiber does not follow a great circle. Nothing landed below the floor, which is the sanity check the runbook asked for.
- **Private peering does not cancel geography.** Arm B2 used the private hostname over VPC peering. Median search was **69.90 ms**, within a few milliseconds of the public path. Peering keeps the traffic off the public internet and changes how you pay for bytes. It does not move New York closer to San Francisco.
- **Asking for more results barely matters when the path is long.** Returning 100 neighbors instead of 5 slowed the local arm from 1.90 ms to **11.37 ms**, because extra payload is visible when the network is already fast. On SFO3 public, k=100 was **70.11 ms** versus 66.97 ms at k=5. The extra documents were a rounding error next to the 67 ms trip.
- **A cold connection multiplies the tax.** The first call on a brand-new connection pays TCP plus TLS setup before the query even runs. Locally that first call was **111.34 ms**. Across the continent it was **717.91 ms** public and **668.87 ms** peered, roughly ten times the pooled search. If your app opens a fresh database connection per request, you pay the geography bill several times, not once.
- **The typical call was also the typical call.** On the distant arms, p50 and p95 sat close together (66.97 vs 69.55 ms on B1). The path was consistently slow, not occasionally slow. That is a placement problem, not a noisy index.

The charts below are rendered from the raw JSON in [anishsingh20/data-locality-tax](https://github.com/anishsingh20/data-locality-tax), not from hand-typed numbers. Re-run `python3 analysis/plot_results.py` and the figures move with the files.

![Study design: one NYC3 Droplet querying three paths against identical pgvector corpora.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-00-study-design.png)
*Figure 0. What actually ran. One client in NYC3, two Managed PostgreSQL 16 + pgvector clusters (NYC3 and SFO3), 100,000 synthetic 768-d vectors, HNSW cosine, 75 measured trials after 10 warmups per cell. Window: 2026-08-10 10:04:37–10:05:45 UTC. Arms B3 and C were not executed and are omitted rather than estimated. Source: live DigitalOcean run, [anishsingh20/data-locality-tax](https://github.com/anishsingh20/data-locality-tax).*

![Bar chart of pooled retrieval p50 at k=5 for Arm A (1.9 ms), Arm B1 (67.0 ms), and Arm B2 (69.9 ms), with a 35× callout from B1 to A.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-01-retrieval-p50-k5.png)
*Figure 1. The headline result. Same query, same index, same corpus. Moving the store from NYC3 to SFO3 multiplied median pooled retrieval by 35×. Source: `a_k5.json`, `b1_k5.json`, `b2_k5.json`.*

![Grouped bars comparing TCP connect p50, pooled retrieval p50, and cold first call across the three arms.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-02-tcp-retrieval-cold.png)
*Figure 2. On the distant arms, pooled retrieval sits on top of the TCP probe. The index is not the bottleneck. Cold first calls (connect + TLS + first query) land around 670–718 ms. Source: matching `*_tcp.json` and `*_k5.json` files.*

![Grouped bars of pooled retrieval p50 at k=5, 20, and 100 for each arm.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-03-topk-payload.png)
*Figure 3. Extra neighbors cost real time only when the network is already fast. Arm A rises from 1.90 ms at k=5 to 11.37 ms at k=100. B1 stays in the mid-60s to low-70s. Source: `*_k5.json`, `*_k20.json`, `*_k100.json`.*

![Bar chart of physics fiber floors versus measured TCP connect p50 for each arm.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-04-floor-vs-measured-tcp.png)
*Figure 4. Sanity check against physics. NYC–SFO cannot beat 41.3 ms in fiber. Measured TCP p50 was 68.63 ms public and 67.73 ms peered, about 27 ms of routing overhead above the floor. Nothing landed below the floor. Source: TCP probes plus the haversine floors in this article.*

![Line chart of summed retrieval tax versus hop count using measured Arm A and Arm B1 p50 values.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-05-compounding-measured.png)
*Figure 5. Arithmetic on the measured k=5 p50, not a second experiment. Eight sequential hops on B1 sum to 536 ms of geography before a token generates. Arm A stays under 16 ms at the same hop count. Source: `a_k5.json` and `b1_k5.json` multiplied by hop count.*

![Grouped bars of retrieval p50 versus p95 at k=5 for each arm.](https://raw.githubusercontent.com/anishsingh20/data-locality-tax/main/figures/fig-06-p50-vs-p95.png)
*Figure 6. The distant path is reliably slow. B1 p95 is only 2.6 ms above p50. That is placement, not a noisy index. Source: pooled summaries in `*_k5.json`.*
...
These are floors multiplied by hop counts, pure arithmetic on the derived numbers, and real totals sit above every cell.

The middle rows are the story: **a continental misplacement costs a third to half a second of pure geography per 8-hop agent task**, and an intercontinental one costs more than a second, before any query executes, before any token generates, on every single task, forever, until someone moves the data.

![Compounding chart showing total geography tax versus number of sequential retrieval hops, with four lines for same-datacenter, NYC to SFO, NYC to Frankfurt, and NYC to Singapore floors, the same-DC line staying flat near zero while the Singapore line passes one second at eight hops.](https://doimages.nyc3.cdn.digitaloceanspaces.com/006Community/2026-Community-Tutorials-Anish/DO-CORE-NEW-CONTENT/AI%20Inference%20Tutorials/data-locality-tax/diagram-dl-4-compounding.png)
...
For a single-retrieval chat query with a 2-second generation, a 62 ms cross-region tax is 3.1 percent of the response time: real, measurable, and survivable, and if this describes your workload, [index tuning](https://www.ibm.com/docs/en/content-cortex/5.6.0?topic=queries-tuning-database-indexes) and caching are better uses of your week than a migration. For an 8-hop agent task whose steps each generate briefly, the 496 ms summed tax is no longer a rounding error: against steps averaging 300 ms of generation each, geography adds roughly 17 percent to the task's critical path, and against shorter tool-selection steps it approaches parity with the compute itself. Both statements are arithmetic on the same derived tax. The workload, not the tax, decides the verdict.

### The pooling confounder, which multiplies the tax for free

A cold connection pays TCP setup, one round trip, then TLS setup, one more round trip on TLS 1.3 and two more on TLS 1.2, before the query sends. This is protocol arithmetic, not measurement. On a same-datacenter path those handshakes cost single-digit milliseconds and nobody notices.

On a 62 ms cross-region path, the identical handshakes cost 124 to 186 ms of pure setup, per connection, and a client misconfigured to open a fresh connection per request, which is a common default in serverless functions and quickly written scripts, pays the setup tax on every call and roughly triples the geography bill. [Connection pooling](https://www.digitalocean.com/community/tutorials/connection-pooling-in-java) is free and removes the entire multiplier, which makes it the single cheapest fix, it does not shrink the tax, it stops you paying it three times.

![Two-lane comparison of a pooled connection paying one round trip per query against a cold connection paying TCP plus TLS handshakes before each query, with the cross-region case showing 124 to 186 ms of setup ahead of the first byte of the query.](https://doimages.nyc3.cdn.digitaloceanspaces.com/006Community/2026-Community-Tutorials-Anish/DO-CORE-NEW-CONTENT/AI%20Inference%20Tutorials/data-locality-tax/diagram-dl-5-pooling.png)
...
Each rung's tax is the derived floor where physics sets it and a template entry where only measurement answers.
...
![Placement hierarchy ladder with five rungs from same-datacenter VPC at the floor through same region, cross-region, third-party SaaS, and object-storage-backed retrieval, each rung annotated with its derived floor or a to-be-measured marker.](https://doimages.nyc3.cdn.digitaloceanspaces.com/006Community/2026-Community-Tutorials-Anish/DO-CORE-NEW-CONTENT/AI%20Inference%20Tutorials/data-locality-tax/diagram-dl-6-hierarchy.png)
...
### Practical guidance, three checks in order

- First, find out where your vector store actually runs, because many teams cannot answer this: for DigitalOcean managed engines the region is explicit at [cluster creation](https://docs.digitalocean.com/products/vector-databases/how-to/create/), and for a SaaS store the region hides in the cluster settings page most people last saw at signup.

- Second, **match regions deliberately across GPU Droplet, vector database, and Knowledge Base** if you use the managed RAG path: the [Knowledge Bases and MCP tutorial](https://www.digitalocean.com/community/tutorials/build-rag-agent-digitalocean-knowledge-bases-mcp) already carries region-placement guidance.

- Third, before provisioning experiment arms, confirm engine availability in your chosen pair on the [availability page](https://docs.digitalocean.com/products/vector-databases/details/availability/), since Weaviate is in public preview and its region list may trail PostgreSQL and OpenSearch. Engine selection itself is covered by the existing [How to Choose the Right Vector Database for Your RAG Architecture](https://www.digitalocean.com/community/conceptual-articles/how-to-choose-the-right-vector-database) tutorial.
...
> Note: The file/python script `locality_bench.py` is in the next section and also present on the Github repository I have created [locality_bench.py](https://github.com/anishsingh20/data-locality-.tax/blob/main/harness/locality_bench.py) for this test.
...
This script is used to run the TCP and pgvector probes on the arms.
...
### 1. Does VPC peering fix cross-region vector DB latency?

No. In my run, NYC3→SFO3 peered private hostname (B2) matched public (B1) within a few milliseconds on TCP and retrieval. Peering changes privacy and egress accounting; it does not move the buildings closer.

### 2. Is ~60–70 ms of continental tax worth a migration for single-shot RAG?

Often no. Against a 2-second generation, a 67 ms retrieval tax is a few percent of response time. Index tuning and caching may be better uses of the week. For multi-hop agentic RAG, the same tax times hop count and the verdict flips in favor of the closer placement.

### 3. Why was my cold pgvector call ~10× the pooled p50?

A cold connection pays TCP plus TLS handshakes before the first query. On a ~68 ms NYC–SFO path, that setup alone can land in the hundreds of milliseconds. [Pool the client](https://www.digitalocean.com/community/tutorials/connection-pooling-in-java); do not open a new connection per request.

### 4. Should the vector database always sit next to the GPU?

Yes when you control both and residency allows it. Same-DC VPC was 1.90 ms pooled k=5 versus 66.97 ms cross-region in my run. Compliance residency can force the opposite move: relocate compute toward the data, or replicate a slowly changing index.
...
The optimizations across this cluster compose or fail together across the whole path. A [warm prompt cache](https://www.digitalocean.com/community/tutorials/prompt-caching-cost-break-even) saves prefill milliseconds a wrong-region vector database hands right back. A [tuned serving tail](https://www.digitalocean.com/community/conceptual-articles/when-your-vllm-p99) means little behind 496 ms of geography, and the [inference trilemma](https://www.digitalocean.com/blog/llm-inference-tradeoffs) framing applies to placement exactly as it applies to serving: you choose which constraint to pay, and this article prices one that most teams never noticed they were paying.

The rule of thumb is: **Co-locate first. Tune second. Measure both.**
